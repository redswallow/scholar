...
<?php
	$uid=$_SESSION['uid'];
	if ($search_key!=null){
	$result=mysql_query("SELECT * FROM paper where title LIKE '%$search_key%' or abstract LIKE '%$search_key%' or year like '%$search_key%' or id in (SELECT paper_id FROM author_has_paper where author_id in (select id from author where name like '%$search_key%'))");
	while($row=mysql_fetch_array($result,MYSQL_NUM)){
		$paper_id=$row[0];$title=$row[1];$abstract=$row[2];$year=$row[3];$url=$row[4];$cite=$row[5];
		//str_replace("$search_key","<font color=red>$search_key</font>",$title);
		//str_replace($search_key,"<font color=red>".$search_key."</font>",$title);
		$title=eregi_replace($search_key,"<font color=red>".$search_key."</font>",$title);
		$abstract=eregi_replace($search_key,"<font color=red>".$search_key."</font>",$abstract);
		$year=eregi_replace($search_key,"<font color=red>".$search_key."</font>",$year);
?>
	<div class="paper" onmouseover="this.style.backgroundColor='#F7F3E8';" onmouseout="this.style.backgroundColor='#FFF';" style="background-color: rgb(255,255,255);" >
		...
		<?php 
			...
			while($author_row=mysql_fetch_array($author_res,MYSQL_NUM)){
				$author=$author_row[0];
				$author=eregi_replace($search_key,"<font color=red>".$search_key."</font>",$author);
				echo ' , ',$author;
			}
		?>
		</span><br />
